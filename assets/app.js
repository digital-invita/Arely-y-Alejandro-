    ...
    const INVITATION_CONFIG =
{
  ...
  "sectionColors": {
    ...
    const editorCustomSectionsList = document.getElementById("editorCustomSectionsList");
    ...
    function addToCalendar(event) {
      ...
      // iPadOS se identifica como "Macintosh" pero expone soporte táctil,
      // así que lo detectamos junto con iPhone/iPad/iPod.
      const isIOS = /iPad|iPhone|iPod/i.test(userAgent)
        || (/Macintosh/i.test(userAgent) && navigator.maxTouchPoints > 1);
      const isMacDesktop = /Mac/i.test(userAgent) && !isIOS;
      ...
      if (isIOS) {
        // Safari en iOS no soporta de forma confiable el atributo "download"
        // sobre un Blob (a veces no hace nada) y además bloquea la
        // navegación directa a URIs "data:" tratándolas como descarga no
        // permitida ("Safari no puede descargar este archivo"). La forma
        // confiable de abrir el diálogo nativo "Agregar evento" es navegar
        // directamente (sin atributo download) a un Blob URL con tipo MIME
        // text/calendar; Safari reconoce el tipo y muestra el panel nativo.
        try {
          const blob = new Blob([icsContent], { type: "text/calendar;charset=utf-8" });
          const blobUrl = URL.createObjectURL(blob);
          window.location.href = blobUrl;
          setTimeout(() => URL.revokeObjectURL(blobUrl), 10000);
          showToast("Toca \"Agregar\" para guardar el evento en tu calendario.");
        } catch (error) {
          console.warn("No se pudo abrir el evento de calendario en iOS.", error);
          showToast("No se pudo abrir el calendario. Intenta de nuevo.");
        }
        return;
      }
