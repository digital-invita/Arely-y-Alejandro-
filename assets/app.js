    ...
    const INVITATION_CONFIG =
{
  ...
  "sectionColors": {
    ...
    function addToCalendar(event) {
      ...
      if (isIOS) {
        // Safari en iOS no permite descargar de forma confiable archivos
        // .ics generados en el cliente (ni con Blob, ni con data:): en
        // ambos casos muestra "Safari no puede descargar este archivo".
        // Es una limitación conocida y documentada de iOS (no un error de
        // este sitio), así que en iOS usamos el enlace web de Google
        // Calendar: no requiere descargar nada, solo abre una URL normal,
        // que es la forma estándar y confiable de hacerlo en Safari.
        const providerUrl = buildGoogleCalendarUrl(title, details, EVENT_LOCATION);
        const opened = openCalendarProvider(providerUrl);
        showToast(opened
          ? "Se abrió Google Calendar. También puedes agregarlo manualmente a tu calendario."
          : "No se pudo abrir el calendario. Intenta de nuevo.");
        return;
      }
